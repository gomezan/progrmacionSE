...
{
   /* El divisor pasa el reloj de 80 MHz a 3.6 kHz */
   TIMERG0.hw_timer[0].config.tx_en=0;
   TIMERG0.hw_timer[0].config.tx_divider=22222;
...
char Atender_timer (char atienda)
   {
   /* 10 veces para tener 3600/10 = 360 Hz */
   uint32_t reload_val = 10;
   ...
   if(((read_lo == 0) && (read_hi == 0)) || (read_lo > reload_val))
   {
      if(atienda)
      {
         TIMERG0.hw_timer[0].config.tx_en=0;
         TIMERG0.hw_timer[0].load.val=1;
         TIMERG0.hw_timer[0].loadlo.val=reload_val;
         TIMERG0.hw_timer[0].loadhi.val=0;
         TIMERG0.hw_timer[0].load.val=0;
         TIMERG0.hw_timer[0].config.tx_en=1;
      }
